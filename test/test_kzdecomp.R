...
# daily signal
# we remove weekly signal with low pass filter
a<-kz_decompose(y, period=c(365,8))
plot(a)
summary(a)

# smooth daily high frequencies for smoothed weekly signal from residuals
b<-kz_decompose(residuals(a), period=c(8,1))
plot(b)
summary(b)
