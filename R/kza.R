...
kza <- function(x,q,kz=NULL,k=3,m=0,tol=1.0e-5) {
    ...
    if (!length(kz)) { 
        kz <- .Call("kz", x, q, k, PACKAGE="kza") 
    }
    x <- .Call("kza", x, kz, q, k, m, tol, PACKAGE="kza")
}
...
kzsv <- function(y=NULL,kza=NULL,kz=NULL,q,k=3,m=0,tol=1.0e-5) {
    ...
    if (!length(kz)) { 
        kz <- .Call("kz", y, q, k, PACKAGE="kza") 
        kza <- .Call("kza", y, kz, q, k, m, tol, PACKAGE="kza") 
    }
    s <- .Call("kzsv", kza, kz, q, k, m, tol, PACKAGE="kza")
    return( s/mean(s))
}
